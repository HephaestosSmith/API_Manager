...
public class SysJob extends BaseEntity
{
    ...
    /** 任務組名 */
    @Schema(description = "任務組名")
    @Excel(name = "任務組名")
    private String jobGroup;

    /** 呼叫目標字串 */
    @Schema(description = "呼叫目標字串")
    @Excel(name = "呼叫目標字串")
    private String invokeTarget;

    /** cron執行表示式 */
    @Schema(description = "執行表示式 ")
    @Excel(name = "執行表示式 ")
    private String cronExpression;

    /** cron計劃策略 */
    @Schema(description = "計劃策略 ")
    @Excel(name = "計劃策略 ", readConverterExp = "0=預設,1=立即觸發執行,2=觸發一次執行,3=不觸發立即執行")
    private String misfirePolicy = ScheduleConstants.MISFIRE_DEFAULT;

    /** 是否併發執行（0允許 1禁止） */
    @Schema(description = "併發執行")
    ...
    @Size(min = 0, max = 500, message = "呼叫目標字串長度不能超過500個字元")
